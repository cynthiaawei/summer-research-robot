...
#     """Enhanced face recognition system with better state management"""
    
#     def __init__(self, images_path=None):
#         # Use backend's facepics directory as default
#         if images_path is None:
#             backend_dir = os.path.dirname(__file__)
#             self.images_path = os.path.join(backend_dir, 'facepics')
#         else:
...
#                 image_files.extend(Path(self.images_path).glob(ext))
...
#         logger.info(f"Successfully generated {len(encodeList)} face encodings")
#         return encodeList
...
#             if image is None:
#                 logger.error("❌ Captured image is None")
#                 return False
...
#     def recognize_single_frame(self, img):
#         """Recognize faces in a single frame - optimized for web interface"""
#         try:
#             if img is None:
#                 return None, 0.0
...
#                     if best_distance < 0.45 and matches[matchIndex]:
...
#         """Enhanced findMatch with web interface support"""
#         if single_frame is not None:
#             # Single frame mode for web interface
#             name, confidence = self.recognize_single_frame(single_frame)
#             return name if name else "Unknown"
        
#         # Original continuous mode for CLI
#         return self._continuous_recognition(mode)
    
#     def _continuous_recognition(self, mode):
#         """Continuous face recognition for CLI mode"""
#         unknown_count = 0
#         known_count = 0
...
#             name, confidence = self.recognize_single_frame(img)
            
#             if name:
#                 known_count += 1
#                 unknown_count = 0
#                 if known_count >= 2:
#                     logger.info(f"✅ User recognized after {known_count} confirmations: {name}")
#                     return name
#             else:
#                 unknown_count += 1
#                 known_count = 0
...
#                     logger.info(f"❌ Recognition attempt {self.recognition_attempts}/{self.max_attempts} failed")
...
#             image_path = os.path.join(self.images_path, f"{clean_name}.jpg")
            
#             if os.path.exists(image_path):
#                 os.remove(image_path)
#                 logger.info(f"✅ Deleted image for user: {username}")
