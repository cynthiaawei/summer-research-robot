...
float accel_x_clean = 0;
float accel_y_clean = 0;
float accel_x_ms2 = 0;
float accel_y_ms2 = 0;

// CRITICAL: Much more aggressive thresholds to stop drift
const float VELOCITY_DECAY = 0.85;        // Was 0.98 - much more aggressive!
const float MIN_VELOCITY_THRESHOLD = 0.002; // Was 0.05,  0.005 - higher to stop drift
const float FORCE_W_ZERO_THRESHOLD = 0.3;
const float ACCEL_THRESHOLD = 0.12;       // Was 0.25,  0.12 - higher to ignore small accelerations
const float MAX_VELOCITY = 1.5;           // Was 1.5 - much lower max velocity
...
const unsigned long STATIONARY_TIME_MS = 100; // 200ms of low accel = stationary

// ACCELERATING: use to tell the robot not to randomly subtract distance when decelerating
bool accelerating[2] = {false, false};
int accelDir[2] = {0, 0};   // use to check directions of accelerations. [0] is x, [1] is y
float accel_ms[2] = {accel_x_ms2, accel_y_ms2};
float maxv[2] = {0, 0}; // max velocity accumulated when accelerating
float vels[2] = {vel_x, vel_y};
...
void loop() {
  ...
  accel_x_clean = filtered_ax - accel_offset_x;
  accel_y_clean = filtered_ay - accel_offset_y;
  accel_x_ms2 = accel_x_clean * 9.81;
  accel_y_ms2 = accel_y_clean * 9.81;
  ...
  // update arrays before calculations
    
  accelerating[0] = fabs(accel_x_ms2) > 0;
  accelerating[1] = fabs(accel_y_ms2) > 0;
  accel_ms[0] = accel_x_ms2;
  accel_ms[1] = accel_y_ms2;
  vels[0] = vel_x;
  vels[1] = vel_y;

  // do the thing with the accelerations

  // case 1: just started acceleration from a = 0
  for(int i = 0; i < 2; i++){
    if(!accelerating[i] && accel_ms[i] != 0){ // check if we started moving
      accelerating[i] = true;
      accelDir[i] = accel_ms[i]/abs(accel_ms[i]); //keep the directions
      maxv[i] = vels[i];
    }
  }
  // if still accelerating, then keep track of max velocity obtained
  for(int i = 0; i < 2 && accel_ms[i] != 0; i++){
    accelDir[i] = accel_ms[i]/abs(accel_ms[i]);
  }
   
  // case 3: was already accelerating
  for(int i = 0; i < 2; i++){
    if(accelerating[i] && (accel_ms[i]/abs(accel_ms[i]) == accelDir[0])){ // still accelerating
      maxv[i] += vels[i];
    } else if(-1*accel_x_ms2/abs(accel_x_ms2) == accelDir[0]){ 
      accelerating[i] = false; // decelerating
      maxv[i] -= vels[i]; 
    }
  }

  // update accelDir with current values
  accelDir[0] = fabs(accel_x_ms2)/accel_x_ms2;
  accelDir[1] = fabs(accel_y_ms2)/accel_y_ms2;
  ...
  if (current_time - last_print_time >= 100) {
    ...
    Serial.print(maxv[0], 3); Serial.print(",");
    Serial.print(maxv[1], 3); Serial.print(",");
