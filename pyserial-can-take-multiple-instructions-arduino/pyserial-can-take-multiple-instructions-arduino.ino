...
String nextWord(String &input) {
  ...
  if (index == -1) {
    isEnd = true;  // No more spaces
    String word = input;
    input = "";    // Clear input
    return word; // return last word
  }

  String word = input.substring(0, index); // return first word
  input = input.substring(index + 1); // remove first word from the string
  return word;
}
...
void processCommand(String command, int time){
  if (command == "forward") {
      goForwards(gSliderSpeed, time);  // Move forward for 5 seconds
  } else if (command == "backward") {
      goBackwards(gSliderSpeed, time);
  }  else if (command == "turnRight") {
      turnRight(gSliderSpeed, time);
  } else if (command == "turnLeft") {
      turnLeft(gSliderSpeed, time);
  } else if (command == "moveRight") {
      moveRight(gSliderSpeed, time);
  } else if (command == "moveLeft") {
      moveLeft(gSliderSpeed, time);
...
void loop() {
    ...
    if (Serial.available()) {
      ...
      command.trim();
      splitString(command, words);

      for (int i = 0; i < 18; i += 2) {
        if (words[i] == "" || words[i + 1] == "") break;
        String subCommand = words[i];
        int value = words[i + 1].toInt();
        processCommand(subCommand, value);
      }
    }
}
