...
async def handle_conversation():
    ...
    while True:
        mode = input("Use (s)peech or (t)ype? ").strip().lower()
        if mode == 's':
            user_input = listen()
            if not user_input:
                continue
        ...
        for instruction in instructions:
            ...
            if 'turn left' in instruction: direction = 'turnLeft'; time_in_ms = 5000
            elif 'turn right' in instruction: direction = 'turnRight'; time_in_ms = 5000

            if direction and time_in_ms:
                long_instruction += f"{direction} {time_in_ms} "
                contain_instructions = True
            elif direction == "stop":
                long_instruction += "stop "
                contain_instructions = True
